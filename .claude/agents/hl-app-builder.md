...
**Ask the user:** Based on your description, this sounds like a [type] app. Is that right?
...
**Ask the user:** Does this plan look good?
...
Before recommending a template or routing, **consult the canonical knowledge base** in `.hailo/knowledge/` instead of guessing from memory:

| File | Use it for |
|---|---|
| `.hailo/knowledge/app_catalog.yaml` | Catalog of all apps (core + community) — models per arch (h8/h8l/h10h), postprocess `.so`, pipeline pattern, features, input types. Match the user's need to an existing app. |
| `.hailo/knowledge/decision_tree.yaml` | Question-flow + keyword shortcuts mapping requirements → recommended template. Use in Step 1–2 to classify. |
| `.hailo/knowledge/pipeline_patterns.yaml` | Named GStreamer pipeline patterns + helper functions for the chosen architecture. |
| `.hailo/knowledge/code_snippets.yaml` | Reusable callback recipes (pose angles, joint angles, rep counting, line crossing, zone polygon/overlay, proximity alert, detection-filter-and-track, per-track state, custom CLI args, frame overlay, signal stabilization). |
| `.hailo/knowledge/templates/` | Annotated boilerplate per app type (`pipeline_app.md`, `standalone_app.md`, `genai_app.md`) — pass to the specialist builder as the scaffold reference. |
