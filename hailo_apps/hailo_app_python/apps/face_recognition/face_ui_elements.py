...
class UIElements(BaseUIElements):
    ...
    def __init__(self):
        ...
        self.lance_db_vector_search_classificaiton_confidence_threshold = gr.Slider(
            minimum=CLASSIFICATION_THRESHOLD_MIN,
            maximum=CLASSIFICATION_THRESHOLD_MAX,
            label="Classificaiton confidence threshold",
            elem_id="classificaiton-confidence-threshold-slider",
        )
        self.skip_frames = gr.Slider(
            minimum=SKIP_FRAMES_MIN,
            maximum=SKIP_FRAMES_MAX,
            label="Frames to skip before trying to recognize",
            elem_id="skip-frames-slider",
        )
        # Text Areas
        self.ui_text_message = gr.TextArea(
            label="Detected Persons", interactive=False, elem_id="detected-persons-textarea"
        )  # ID for custom styling
    ...
    def create_interface(self, ui_callbacks, pipeline):
        custom_theme = CustomTheme().set(
            loader_color=SECONDARY_HUE_COLOR, slider_color=SECONDARY_HUE_COLOR
        )
        ...
        with gr.Blocks(css=self.ui_css, theme=custom_theme) as interface:
            ...
            with gr.Row():
                gr.Markdown(
                    "## Live Video Stream, Embeddings Visualization & Parameters tuning",
            ...
            with gr.Row():
                with gr.Column(
                    elem_classes=["fixed-size"]
                ):  # Apply fixed size for live_video_stream
                    self.live_video_stream.render()
                with gr.Column(
                    elem_classes=["fixed-size"]
                ):  # Apply fixed size for embeddings_stream
            ...
            with gr.Row():
                ...
                original_file = get_resource_path(
                    pipeline_name=None,
                    resource_type=DEFAULT_LOCAL_RESOURCES_PATH,
                    model=HAILO_LOGO_PHOTO_NAME,
                )
            ...
            self.start_btn.click(
                fn=ui_callbacks.process_ui_text_message, inputs=None, outputs=self.ui_text_message
            )

            self.save_btn.click(fn=ui_callbacks.save_algo_params, inputs=None, outputs=None)

            if pipeline.options_menu.visualize:
                interface.load(
                    fn=ui_callbacks.consume_plot_queue, inputs=None, outputs=self.embeddings_stream
                )

            # Dynamically adjust initial values for sliders from pipeline
            self.lance_db_vector_search_classificaiton_confidence_threshold.value = (
                pipeline.lance_db_vector_search_classificaiton_confidence_threshold
            )
            self.skip_frames.value = pipeline.skip_frames

            self.lance_db_vector_search_classificaiton_confidence_threshold.change(
                ui_callbacks.on_lance_db_vector_search_classificaiton_confidence_threshold_change,
                inputs=self.lance_db_vector_search_classificaiton_confidence_threshold,
            )
...
class CustomTheme(gr.themes.Default):
    def __init__(self):
        ...
        self.button_primary_background_fill = (
            f"linear-gradient(90deg, {PRIMARY_HUE_COLOR} 0%, {SECONDARY_HUE_COLOR} 100%)"
        )
        ...
        self.button_primary_background_fill_hover = (
            f"linear-gradient(90deg, {SECONDARY_HUE_COLOR} 0%, {PRIMARY_HUE_COLOR} 100%)"
        )
