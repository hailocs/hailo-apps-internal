...
static const std::vector<hailo_scalar_t> color_table = {
    hailo_scalar_t(255, 0, 0), hailo_scalar_t(0, 255, 0), hailo_scalar_t(0, 0, 255), hailo_scalar_t(255, 255, 0), hailo_scalar_t(0, 255, 255),
    hailo_scalar_t(255, 0, 255), hailo_scalar_t(255, 170, 0), hailo_scalar_t(255, 0, 170), hailo_scalar_t(0, 255, 170), hailo_scalar_t(170, 255, 0),
    hailo_scalar_t(170, 0, 255), hailo_scalar_t(0, 170, 255), hailo_scalar_t(255, 85, 0), hailo_scalar_t(85, 255, 0), hailo_scalar_t(0, 255, 85),
    hailo_scalar_t(0, 85, 255), hailo_scalar_t(85, 0, 255), hailo_scalar_t(255, 0, 85), hailo_scalar_t(255, 255, 255)};
...
static overlay_status_t draw_conf_class_mask(cv::Mat &image_planes, HailoConfClassMaskPtr mask, HailoROIPtr roi, const uint mask_overlay_n_threads)
{
    ...
    hailo_scalar_t mask_color = indexToColor(mask->get_class_id());
...
static bool try_get_custom_color(HailoDetectionPtr det, hailo_scalar_t &out)
{
    for (auto &obj : det->get_objects()) {
        ...
        if (cid > 0) {
            out = hailo_scalar_t((cid >> 16) & 0xFF, (cid >> 8) & 0xFF, cid & 0xFF);
            return true;
        }
        // Fallback: parse "R,G,B" from label
        int r, g, b;
        if (sscanf(cls->get_label().c_str(), "%d,%d,%d", &r, &g, &b) == 3) {
            out = hailo_scalar_t(r, g, b);
            return true;
        }
    }
    return false;
        ...
        case HAILO_DETECTION:
        {
            ...
            if (draw_label && !bbox_replaced_by_sprite && !text.empty()) {
                ...
                if (params.text_background) {
                    ...
                    cv::rectangle(mat, bg_rect, cv::Scalar(0, 0, 0), cv::FILLED);
                }
            ...
            if (params.sprite_cache) {
                ...
                if (!sprite_key.empty()) {
                    ...
                    if (sprite)
                        draw_sprite(mat, cv::Rect(rect.x, rect.y, rect.width, rect.height), *sprite);
                }
            }
            break;
        }
