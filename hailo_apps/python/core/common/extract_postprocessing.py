...
def extract_hef_like_intermediate_model(input_model_path, output_model_path, intermediate_output_names):
    ...
    Returns:
        None (saves modified model to output_model_path)
    """
    # Load the full model and run shape inference so value_info is populated
    model = onnx.load(input_model_path)
    try:
        model = onnx.shape_inference.infer_shapes(model)
        print("Shape inference completed successfully")
    except Exception as e:
        print(f"Shape inference warning: {e} (shapes may be missing from outputs)")
    ...
    # Try to get shape info from the original model's value_info first
    value_info_map = {}
    ...
    for tensor_name in intermediate_output_names:
        if tensor_name in value_info_map:
            # Use existing value_info (has shape and type)
            new_outputs.append(value_info_map[tensor_name])
            print(f"  Output '{tensor_name}': shape from value_info")
        else:
            # Fallback: run shape inference on the original model to get shapes
            print(f"  Output '{tensor_name}': not in value_info, creating with unknown shape")
            new_output = helper.make_tensor_value_info(
                tensor_name,
    ...
    # Prune postprocessing nodes: keep only nodes needed to produce intermediate outputs.
    # Backward traversal from intermediate outputs to find all required nodes.
    required_tensors = set(intermediate_output_names)
    required_node_names = set()
    ...
    # Collect all initializer names (these are "free" - no node needed)
    initializer_names = {init.name for init in model.graph.initializer}
    # Also collect graph input names
    graph_input_names = {inp.name for inp in model.graph.input}
    
    # BFS backward from required tensors
    tensors_to_trace = list(required_tensors)
    traced_tensors = set()
    
    while tensors_to_trace:
        tensor = tensors_to_trace.pop()
        if tensor in traced_tensors:
            continue
        traced_tensors.add(tensor)
        
        # Skip if this tensor is a model input or initializer (no producing node)
        if tensor in initializer_names or tensor in graph_input_names:
            continue
        ...
        required_node_names.add(producer.name)
        # Trace backward through this node's inputs
        for inp in producer.input:
            if inp not in traced_tensors:
                tensors_to_trace.append(inp)
    
    original_count = len(model.graph.node)
    pruned_nodes = [node for node in model.graph.node if node.name in required_node_names]
    pruned_count = original_count - len(pruned_nodes)
    
    print(f"Pruned {pruned_count} postprocessing nodes ({original_count} -> {len(pruned_nodes)})")
    ...
    for node in pruned_nodes:
        for inp in node.input:
            used_initializers.add(inp)
    pruned_initializers = [init for init in model.graph.initializer if init.name in used_initializers]
    
    # Rebuild graph with pruned nodes
    new_graph = helper.make_graph(
        pruned_nodes,
        model.graph.name + "_intermediate",
        list(model.graph.input),  # Keep original inputs
        new_outputs,
        pruned_initializers
    )
    
    # Copy value_info for remaining internal tensors
    for vi in model.graph.value_info:
        if vi.name in traced_tensors:
            new_graph.value_info.append(vi)
    
    new_model = helper.make_model(new_graph, producer_name="onnx-intermediate-extractor")
    # Copy opset from original
    del new_model.opset_import[:]
