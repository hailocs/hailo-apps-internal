...
def OVERLAY_PIPELINE(name="hailo_overlay", community=False):
    ...
    overlay_pipeline = f"{QUEUE(name=f'{name}_q')} ! {element} name={name} "
...
def DISPLAY_PIPELINE(
    ...
    display_pipeline = (
        f"{OVERLAY_PIPELINE(name=f'{name}_overlay', community=community_overlay)} ! "
