...
└── setup/                        # Setup scripts and generated files
    ...
    ├── generate_token_embedding_lut_openai_clip.py  # Step 2: Generate token embeddings
    ├── generate_text_projection_openai_clip.py      # Step 3: Generate text projection
    ├── build_sample_embeddings_json.py            # Step 4: Generate sample JSON files
...
3. **Setup Scripts** (in `setup/`):
   - All generator scripts for one-time setup
   - Generated files (tokenizer, embeddings, projection matrix)
...
These files are required for the text encoder to work and are automatically loaded by `clip_text_utils.py`.
