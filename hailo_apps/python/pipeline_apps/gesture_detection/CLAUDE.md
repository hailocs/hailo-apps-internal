...
| File | Purpose |
|------|---------|
| `gesture_detection_gst.py` | GStreamer pipeline app — inference in Python callback, Hailo metadata output |
| `gesture_detection_cpp_pipeline.py` | Full C++ GStreamer pipeline — all pre/post in C++ hailofilter elements |
| `gesture_detection_h8.py` | Python standalone (OpenCV) — no GStreamer, good for debugging |
...
| `download_blaze_models.py` | Downloads HEF model files to `models/` |
| `BENCHMARK_REPORT.md` | Detailed benchmark results (RPi 5 + x86) |

## Pipeline Variants

### 1. GStreamer + Python inference (`gesture_detection_gst.py`)
```
...
### 4. Python standalone (`gesture_detection_h8.py`)
```
...
```python
# GStreamer pipeline (gesture_detection_gst.py) attaches standard Hailo metadata:
...
Download: `python -m hailo_apps.python.pipeline_apps.gesture_detection.download_blaze_models`
...
- **Integrate into another pipeline:** Use `gesture_detection_gst.py` as a reference — the `app_callback()` returns standard Hailo metadata consumable by any downstream GStreamer element
