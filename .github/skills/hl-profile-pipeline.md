# Skill: Profile GStreamer Pipeline Performance

> Profile a GStreamer pipeline with GST-Shark, analyze bottlenecks visually, suggest and apply optimizations, then iterate against the user's actual requirements (FPS / latency / CPU). Guides the user through everything.

## When to Use This Skill

- User wants to **profile** a Hailo GStreamer pipeline app (FPS, latency, CPU, bottlenecks)
- Pipeline is **slow, laggy, or unstable** and the cause is unclear
- User wants to **optimize** for specific requirements (low latency, CPU headroom, stable FPS)
- User wants an **A/B comparison** between two pipeline configurations
- User wants to **find the optimal frame rate** for their use case
- User wants to **query past optimization recipes** (knowledge base)
...
```bash
python .github/scripts/profile_pipeline/setup_check.py --json
```
...
```bash
python .github/scripts/profile_pipeline/profile_pipeline.py <app_path> --duration <N> [-- <extra_args>]
```
...
```bash
python .github/scripts/profile_pipeline/analyze_trace.py <trace_dir> --format json
```
```bash
python .github/scripts/profile_pipeline/plot_graphs.py <trace_dir>
```
...
```bash
python .github/scripts/profile_pipeline/knowledge_base.py query --element <top_bottleneck_element>
```
...
3. **Generate charts for the experiment trace** (always!):
   ```bash
   python .github/scripts/profile_pipeline/plot_graphs.py <experiment_trace_dir>
   ```
4. **Compare** baseline vs experiment:
   ```bash
   python .github/scripts/profile_pipeline/compare_traces.py <baseline_dir> <experiment_dir> --format json
   ```
...
```bash
python .github/scripts/profile_pipeline/knowledge_base.py add-recipe \
    --app <app_name> \
    --change "<description>" \
...
Also update the repo memory file at `.github/memory/pipeline_optimization.md`.
...
```bash
python .github/scripts/profile_pipeline/find_best_framerate.py <app_path> \
...
```bash
python .github/scripts/profile_pipeline/knowledge_base.py query --tags <app_name>
```
...
All at `.github/scripts/profile_pipeline/`:
