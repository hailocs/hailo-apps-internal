...
description: Build voice applications for Hailo-8/8L/10H with speech-to-text
  (Whisper on Hailo) and optional text-to-speech (Piper on CPU). Supports STT-only on
  Hailo-8/8L via InferModel API, and full voice+LLM/VLM assistants on Hailo-10H.
...
<!-- INTERACTION: Target hardware?
     OPTIONS: Hailo-10H (full voice + LLM/VLM assistant) | Hailo-8/8L (speech-to-text, optional TTS) | Auto-detect at runtime -->

<!-- INTERACTION: What kind of voice app?
     OPTIONS: Voice + LLM Assistant (H10 only) | Voice + VLM Assistant (H10 only) | Speech-to-Text Only (any Hailo) | Add Voice to Existing App -->
