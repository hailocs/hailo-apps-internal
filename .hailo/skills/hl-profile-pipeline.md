...
```bash
python .hailo/scripts/profile_pipeline/setup_check.py --json
```
...
```bash
python .hailo/scripts/profile_pipeline/profile_pipeline.py <app_path> --duration <N> [-- <extra_args>]
```
...
```bash
python .hailo/scripts/profile_pipeline/analyze_trace.py <trace_dir> --format json
```
```bash
python .hailo/scripts/profile_pipeline/plot_graphs.py <trace_dir>
```
...
```bash
python .hailo/scripts/profile_pipeline/knowledge_base.py query --element <top_bottleneck_element>
```
...
3. **Generate charts for the experiment trace** (always!):
   ```bash
   python .hailo/scripts/profile_pipeline/plot_graphs.py <experiment_trace_dir>
   ```
4. **Compare** baseline vs experiment:
   ```bash
   python .hailo/scripts/profile_pipeline/compare_traces.py <baseline_dir> <experiment_dir> --format json
   ```
...
```bash
python .hailo/scripts/profile_pipeline/knowledge_base.py add-recipe \
    --app <app_name> \
    --change "<description>" \
...
```bash
python .hailo/scripts/profile_pipeline/find_best_framerate.py <app_path> \
...
```bash
python .hailo/scripts/profile_pipeline/knowledge_base.py query --tags <app_name>
```
...
All at `.hailo/scripts/profile_pipeline/`:
